...
export default function CustomerDetailScreen() {
  ...
  const [c, setC] = useState<any>(() => caseStore.get());

  useEffect(() => {
    // If store has data for this id, use it
    const stored = caseStore.get();
    if (stored && String(stored.id) === String(id)) {
      setC(stored);
    }
  }, [id]);

  // ✅ Show a clear error if no data — never silent blank screen
  if (!c) {
    return (

...
function CaseCard({ item, onFeedback }: { item: any; onFeedback: (item: any) => void }) {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* ── Card Actions: Call + Details always shown; Feedback + Online Collection only when uploaded ── */}
      ...
      {/* ── Online Collection button — only for unpaid/PTP cases ── */}
      {isUploaded && item.status !== "Paid" && (
        <Pressable
          style={styles.onlineCollectBtn}
          onPress={() => router.push({ pathname: "/(app)/online-collection", params: { loanNo: item.loan_no, customerName: item.customer_name } } as any)}
        >
          <Ionicons name="card" size={15} color={Colors.success} />
          <Text style={styles.onlineCollectText}>Collected Online</Text>
        </Pressable>
      )}
...
const styles = StyleSheet.create({
  ...
  onlineCollectBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 9, borderRadius: 10, borderWidth: 1.5, borderColor: Colors.success + "60", backgroundColor: Colors.success + "10", marginTop: 4 },
  onlineCollectText: { fontSize: 13, fontWeight: "700", color: Colors.success },

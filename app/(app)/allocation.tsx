...
import {
  ...
  Alert, ActivityIndicator, Modal, ScrollView, Platform, Animated,
} from "react-native";
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={callStyles.overlay}>
        <View style={callStyles.card}>
          {/* Pulse ring */}
          <Animated.View style={[callStyles.pulseRing, { transform: [{ scale: pulseAnim }] }]} />
          <View style={callStyles.avatarCircle}>
            <Ionicons name="call" size={30} color="#fff" />
          </View>

          <Text style={callStyles.statusLabel}>Calling...</Text>
          <Text style={callStyles.customerName} numberOfLines={1}>{customerName}</Text>
          <Text style={callStyles.phoneNumber}>{phoneNumber}</Text>

          {callSid && (
            <View style={callStyles.sidBadge}>
              <Text style={callStyles.sidText}>SID: {callSid.slice(0, 16)}...</Text>
            </View>
          )}

          <View style={callStyles.infoRow}>
            <Ionicons name="mic" size={13} color={Colors.success} />
            <Text style={callStyles.infoText}>Call is being recorded</Text>
          </View>
          <View style={callStyles.infoRow}>
            <Ionicons name="logo-google" size={13} color={Colors.info} />
            <Text style={callStyles.infoText}>Recording saves to Google Drive</Text>
          </View>

          <Pressable style={callStyles.closeBtn} onPress={onClose}>
            <Ionicons name="close" size={16} color={Colors.textSecondary} />
            <Text style={callStyles.closeBtnText}>Dismiss</Text>
          </Pressable>
        </View>
      </View>
    </Modal>
...
function FeedbackModal({
  visible, caseItem, onClose, onSave, isLocked = false,
}: any) {
  ...
  const [paidRollbackYn,    setPaidRollbackYn]    = useState<boolean | null>(caseItem?.rollback_yn != null ? Boolean(caseItem.rollback_yn) : null);
  ...
  const save = async () => {
    ...
    let finalStatus = activeTab === "Paid" ? "Paid"
      : activeTab === "PTP" ? "PTP"
      : "Unpaid";
    setLoading(true);
    try {
    ...
    <View style={{ gap: 6, marginBottom: 12 }}>
      {options.map((opt) => (
        ...
        >
          ...
          {val === opt && <Ionicons name="checkmark-circle" size={18} color={activeColor} />}
        </Pressable>
      ))}
    </View>
  );
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={fbStyles.overlay}>
        <View style={fbStyles.sheet}>
          ...
          {/* Tab selector — flexWrap fixes Android font rendering bug */}
...
function CaseCard({ item, onFeedback }: { item: any; onFeedback: (item: any) => void }) {
  ...
  const handleTwilioCall = async () => {
    ...
    try {
      const result = await initiateTwilioCall(num, item.customer_name, item.id, item.loan_no || "");
      setCallSid(result.callSid);
      setCallModalVisible(true);
    } catch (err: any) {
      // Fallback to native dialer if Twilio fails
      Alert.alert(
        "Twilio Unavailable",
        "Falling back to native dialer. Call will not be recorded.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Call Anyway", onPress: () => Linking.openURL(`tel:${num}`) },
        ]
      );
    } finally {
      setCalling(false);
  ...
  return (
    <>
      <View style={styles.card}>
        <Pressable style={styles.cardTapArea} onPress={() => navigateToDetail(item)}>
          <View style={styles.cardHeader}>
            <View style={styles.cardNameRow}>
              <View style={styles.cardAvatarSmall}>
        ...
        {/* Phone row */}
        {item.mobile_no && (
          <Pressable style={styles.phoneRow} onPress={handleNativeCall}>
            <Ionicons name="call" size={13} color={Colors.info} />
            <Text style={styles.phoneText}>{item.mobile_no}</Text>
            <View style={styles.recordingBadge}>
              <Ionicons name="mic" size={10} color={Colors.success} />
              <Text style={styles.recordingBadgeText}>RECORD</Text>
            </View>
          </Pressable>
        )}
        ...
        <View style={styles.cardActions}>
          {/* Twilio recorded call */}
          <Pressable
            style={[styles.actionBtn, styles.callBtn, calling && { opacity: 0.7 }]}
            onPress={handleTwilioCall}
            disabled={calling}
          >
            {calling
            ...
            <Text style={styles.actionBtnText}>{calling ? "Calling…" : "Call"}</Text>
            {!calling && (
      ...
      <CallStatusModal
        ...
        phoneNumber={item.mobile_no?.split(",")[0]?.trim() || ""}
        callSid={callSid}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      <View style={styles.recordingBanner}>
        <Ionicons name="mic-circle" size={16} color={Colors.success} />
        <Text style={styles.recordingBannerText}>
          Calls via the <Text style={{ fontWeight: "800" }}>Call</Text> button are recorded & saved to Google Drive
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  tab: {
    ...
    paddingHorizontal: 4, borderRadius: 10,
  ...
  recordingBannerText: { fontSize: 12, color: Colors.success, flex: 1 },
  ...
  phoneText: { fontSize: 13, color: Colors.info, fontWeight: "500", flex: 1 },
  recordingBadge: {
    flexDirection: "row", alignItems: "center", gap: 3,
    backgroundColor: Colors.success + "15", borderRadius: 6,
    paddingHorizontal: 6, paddingVertical: 2,
  },
  recordingBadgeText: { fontSize: 9, fontWeight: "800", color: Colors.success, letterSpacing: 0.5 },
...
const fbStyles = StyleSheet.create({
  ...
  sheet: {
    ...
    padding: 24, maxHeight: "92%", flexShrink: 1,
  },
  ...
  commentInput: {
    ...
    textAlignVertical: "top", backgroundColor: Colors.surfaceAlt, marginBottom: 12,
  },
...
const callStyles = StyleSheet.create({
  ...
  card: {
    width: 300, backgroundColor: Colors.surface, borderRadius: 24,
  ...
  pulseRing: {
    position: "absolute", top: 20, width: 90, height: 90, borderRadius: 45,
    backgroundColor: Colors.primary + "25", borderWidth: 2, borderColor: Colors.primary + "40",
  },
  avatarCircle: {
  ...
  statusLabel:   { fontSize: 13, color: Colors.textMuted, fontWeight: "600", letterSpacing: 1 },
  customerName:  { fontSize: 18, fontWeight: "800", color: Colors.text, textAlign: "center" },
  phoneNumber:   { fontSize: 14, color: Colors.textSecondary, marginBottom: 4 },
  sidBadge: {
    backgroundColor: Colors.surfaceAlt, borderRadius: 8,
    paddingHorizontal: 10, paddingVertical: 4,
    borderWidth: 1, borderColor: Colors.border,
  },
  sidText:  { fontSize: 10, color: Colors.textMuted, fontWeight: "600" },

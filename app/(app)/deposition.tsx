...
function BulkPaymentSheet({ visible, selectedItems, onClose, onPaid }: {
  ...
  const reset = () => { setMode("select"); setCashAmt(""); setOnlineAmt(""); setScreenshotUri(null); };
  ...
  const handleOnline = async () => {
    ...
    try {
      ...
      for (const item of selectedItems) await payOnline(item.id, result.assets[0].uri);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      onPaid(); reset(); onClose();
    } catch (e: any) { Alert.alert("Error", e.message); }
    finally { setLoading(false); }
  ...
  const handleBoth = async () => {
    ...
    const proceed = async () => {
      ...
      try {
        ...
        for (const item of selectedItems) {
          ...
          await payBoth(item.id, Math.round(cash * prop * 100) / 100, Math.round(online * prop * 100) / 100, screenshotUri!);
        }
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        onPaid(); reset(); onClose();
      } catch (e: any) { Alert.alert("Error", e.message); }
    ...
    if (diff > 1) {
      Alert.alert("Amount Mismatch", `Cash ₹${fmt(cash)} + Online ₹${fmt(online)} = ₹${fmt(cash + online)}\nExpected: ₹${fmt(totalAmount)}\n\nProceed anyway?`,
        ...
        <View style={pay.sheet}>
            ...
            <>
              ...
              <View style={pay.optRow}>
                <Pressable style={[pay.opt, { borderColor: Colors.success }]} onPress={() => setMode("cash")}>
                  ...
                  <Text style={pay.optHint}>Full amount</Text>
                </Pressable>
                <Pressable style={[pay.opt, { borderColor: "#2563eb" }]} onPress={handleOnline} disabled={loading}>
                  <View style={[pay.optIcon, { backgroundColor: "#2563eb18" }]}>
                    {loading ? <ActivityIndicator color="#2563eb" /> : <Ionicons name="phone-portrait-outline" size={26} color="#2563eb" />}
                  </View>
          ...
          {mode === "cash" && (
            <>
              ...
              <View style={[pay.amtRow, { borderColor: Colors.success }]}>
                ...
                <TextInput style={pay.amtInput} placeholder="0" placeholderTextColor={Colors.textMuted}
                  value={cashAmt} onChangeText={setCashAmt} keyboardType="numeric" autoFocus />
              </View>
              <Text style={pay.hint}>Total assigned: ₹{fmt(totalAmount)}</Text>
              <View style={pay.btnRow}>
                <Pressable style={pay.backBtn} onPress={() => setMode("select")}>
                  <Ionicons name="arrow-back" size={18} color={Colors.textSecondary} />
                  <Text style={pay.backText}>Back</Text>
                </Pressable>
                <Pressable style={[pay.confirmBtn, { backgroundColor: Colors.success }, loading && { opacity: 0.6 }]} onPress={handleCash} disabled={loading}>
                  {loading ? <ActivityIndicator color="#fff" size="small" /> : <><Ionicons name="checkmark" size={18} color="#fff" /><Text style={pay.confirmText}>Confirm Cash</Text></>}
                </Pressable>
              </View>
            </>
          )}
          ...
          {mode === "both" && (
            <>
              ...
              <Text style={pay.hint}>Total: ₹{fmt((parseFloat(cashAmt) || 0) + (parseFloat(onlineAmt) || 0))} / ₹{fmt(totalAmount)}</Text>
              <Pressable style={[pay.screenshotPicker, screenshotUri && { borderStyle: "solid", borderColor: "#2563eb" }]} onPress={pickScreenshot}>
                {screenshotUri ? (
              ...
              <View style={pay.btnRow}>
                ...
                <Pressable style={[pay.confirmBtn, loading && { opacity: 0.6 }]} onPress={handleBoth} disabled={loading}>
                  {loading ? <ActivityIndicator color="#fff" size="small" /> : <><Ionicons name="checkmark" size={18} color="#fff" /><Text style={pay.confirmText}>Confirm Split</Text></>}
                </Pressable>
              </View>
            </>
          )}
...
function DepositionCard({ item, isSelected, onSelect }: {
  ...
  // ✅ NEW: remaining = assigned - (cash + online paid so far)
  ...
  return (
    <Pressable
      ...
      disabled={!isPending} activeOpacity={isPending ? 0.7 : 1}
    >
      ...
      {/* ✅ NEW: Remaining amount indicator */}
      {hasRemaining && (
      ...
      {isPending && (
        <View style={card.tapHint}>
          <Ionicons name={isSelected ? "checkbox" : "square-outline"} size={14} color={isSelected ? Colors.primary : Colors.textMuted} />
...
export default function FosDepositionScreen() {
  ...
  const { data: depData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) { let msg = `HTTP ${res.status}`; try { msg = JSON.parse(text).message || msg; } catch {} throw new Error(msg); }
  ...
  // ✅ NEW: total remaining across all depositions
  ...
  const selectAll = useCallback(() => {
    setSelectedIds(selectedIds.size === pendingDepositions.length ? new Set() : new Set(pendingDepositions.map((d) => d.id)));
  ...
  if (error) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: Colors.background, padding: 24 }}>
        ...
        <Pressable style={{ marginTop: 20, backgroundColor: Colors.primary, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12 }} onPress={() => refetch()}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      <FlatList
            ...
            <View style={{ gap: 12, marginBottom: 4 }}>
              ...
              {pendingCount > 0 && (
                <Pressable style={main.selectAllBtn} onPress={selectAll}>
                  <Ionicons name={selectedIds.size === pendingCount ? "checkbox" : "square-outline"} size={18} color={Colors.primary} />
        ...
        ListEmptyComponent={
          <View style={main.empty}>
            ...
            <Pressable style={{ marginTop: 16, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10, borderWidth: 1, borderColor: Colors.border }} onPress={() => refetch()}>
      ...
      {/* ✅ NEW: Amount Entry Modal */}
      <Modal visible={!!amountEntryItem} transparent animationType="slide" onRequestClose={() => { setAmountEntryItem(null); setEnteredAmount(""); }}>
        <View style={pay.overlay}>
          <View style={pay.sheet}>
            <View style={pay.handle} />
            {amountEntryItem && (() => {
              ...
              return (
                <>
                  ...
                  <View style={pay.infoCard}>
                    ...
                    <View style={{ alignItems: "flex-end" }}>
                      ...
                      {totalCollectedSoFar > 0 && (
                        <Text style={{ fontSize: 12, color: Colors.success, fontWeight: "600" }}>Collected ₹{fmt(totalCollectedSoFar)}</Text>
                      )}
                      <Text style={{ fontSize: 13, color: Colors.warning, fontWeight: "700" }}>Remaining ₹{fmt(remaining)}</Text>
                    </View>
                  </View>
                  ...
                  >
                    <Text style={{ fontSize: 12, fontWeight: "700", color: Colors.primary }}>Use full remaining ₹{fmt(remaining)}</Text>
                  </Pressable>
                  <View style={pay.btnRow}>
                    <Pressable style={pay.backBtn} onPress={() => { setAmountEntryItem(null); setEnteredAmount(""); }}>

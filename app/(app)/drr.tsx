    ...
    <View style={[s.bktCard, { borderLeftColor: color }]}>
      ...
      <View style={s.bktHead}>
        ...
        <View style={[s.statusPill, {
          backgroundColor: achieved ? Colors.success + "20" : Colors.warning + "20"
        }]}>
          <Ionicons
            name={achieved ? "checkmark-circle" : "time-outline"}
            size={13}
            color={achieved ? Colors.success : Colors.warning}
          />
    ...
    >
        ...
        {MILESTONES.map((m) => {
          ...
          return (
            <View key={m.day} style={[
              s.tableRow,
              isCurrent && { backgroundColor: Colors.accent + "12" },
              isPast     && { opacity: 0.4 },
            ]}>
              <View style={[s.tdCell, { flex: 1.6, flexDirection: "row", alignItems: "center", gap: 5 }]}>
                {isCurrent && <View style={s.activeDot} />}
                <Text style={[s.tdText, isCurrent && { color: Colors.accent, fontWeight: "800" }]}>
                  Day {m.day}
                </Text>
              </View>
      ...
      {(["bkt1", "bkt2", "bkt3"] as const).map((bkt) => (
        <BktCard
          key={bkt}
          bkt={bkt}
          paid={bktMap[bkt].paid}
          total={bktMap[bkt].total}
          today={today}
        />
      ))}

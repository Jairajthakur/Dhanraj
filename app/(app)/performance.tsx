...
function SummaryCard({ row }: { row: any }) {
  ...
  // TW = Total Workable = count_total
  const twTotal  = parseInt(row.count_total  || 0);
  const twPaid   = parseInt(row.count_paid   || 0);
  const twUnpaid = parseInt(row.count_unpaid || 0);

  const requiredRb =
    targets && targets.rollback != null && rbGrandTotal > 0
      ...
      <View style={[styles.card, { borderLeftColor: color }]}>
        ...
        <View style={styles.triRow}>
          <View style={styles.triCell}>
            ...
            <Text style={[styles.triVal, { color: Colors.success }]}>
          ...
          <View style={styles.triCell}>
            ...
            <Text style={[styles.triVal, {
              color: penalRequired !== null && penalRequired <= 0 ? Colors.success : Colors.danger,
              fontSize: 14,
            }]}>
  ...
  return (
    <View style={[styles.card, { borderLeftColor: color }]}>
      ...
      {/* TW — Total Workable cases */}
      <Text style={styles.sectionTitle}>TW (Total Workable)</Text>
      <View style={styles.triRow}>
        <View style={styles.triCell}>
          <Text style={styles.triLabel}>Paid</Text>
          <Text style={[styles.triVal, { color: Colors.success }]}>{twPaid}</Text>
        </View>
        <View style={styles.triCell}>
          <Text style={styles.triLabel}>Unpaid</Text>
          <Text style={[styles.triVal, { color: Colors.danger }]}>{twUnpaid}</Text>
        </View>
        <View style={styles.triCell}>
          <Text style={styles.triLabel}>Total</Text>
          <Text style={[styles.triVal, { color }]}>{twTotal}</Text>
        </View>
      </View>
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[
        styles.container,
        { paddingTop: Platform.OS === "web" ? 67 : 16, paddingBottom: insets.bottom + 24 },
      ]}
    >
...
const styles = StyleSheet.create({
  ...
  targetChip: {
    fontSize: 10, fontWeight: "700", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6,
  },

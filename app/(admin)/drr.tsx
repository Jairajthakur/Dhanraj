...
import {
  ...
  RefreshControl, ActivityIndicator, ScrollView, Platform,
} from "react-native";
...
// ─── Build per-agent DRR from admin bkt-perf-summary rows ────────────────────
function buildAgentDRR(rows: any[], today: number) {
  const agentMap: Record<string, {
    fosName: string;
    bkts: Record<string, { paid: number; total: number }>;
  }> = {};

  for (const row of rows) {
    const name = row.fos_name || "Unknown";
    if (!agentMap[name]) {
      agentMap[name] = { fosName: name, bkts: { bkt1: { paid: 0, total: 0 }, bkt2: { paid: 0, total: 0 }, bkt3: { paid: 0, total: 0 } } };
    }
  ...
  return Object.values(agentMap).map((agent) => {
    ...
    const bktDetails: Record<string, {
      paid: number; total: number; currentPct: number;
      targetPct: number; requiredPos: number; achieved: boolean;
    }> = {};
...
function AgentDRRCard({ item, today }: { item: ReturnType<typeof buildAgentDRR>[0]; today: number }) {
  ...
  return (
    <View style={ac.card}>
      ...
      {/* BKT mini summary (always visible) */}
          ...
          return (
            <View key={bkt} style={[ac.bktMini, { borderColor: bktColor + "40" }]}>
              <View style={ac.bktMiniHeader}>
                ...
                <Text style={[ac.bktMiniPct, { color: d.achieved ? Colors.success : Colors.text }]}>
                  {pct(d.currentPct)}
                </Text>
              </View>
        ...
        <View style={ac.expandedWrap}>
            ...
            return (
              <View key={bkt} style={[ac.bktDetail, { borderLeftColor: bktColor }]}>
                ...
                <View style={ac.bktDetailStats}>
                  <View style={ac.bktDetailStat}>
                    <Text style={ac.bktDetailStatLabel}>COLLECTED</Text>
                    <Text style={[ac.bktDetailStatValue, { color: Colors.success }]}>{fmt(d.paid)}</Text>
                  </View>
                  <View style={ac.bktDetailStat}>
                    <Text style={ac.bktDetailStatLabel}>TOTAL POS</Text>
                    <Text style={ac.bktDetailStatValue}>{fmt(d.total)}</Text>
                  </View>
                  <View style={ac.bktDetailStat}>
                    <Text style={ac.bktDetailStatLabel}>STILL NEED</Text>
                    <Text style={[ac.bktDetailStatValue, { color: d.achieved ? Colors.success : Colors.danger }]}>
                      {d.achieved ? "✓ Done" : fmt(d.requiredPos)}
                    </Text>
                  </View>
                </View>
                <MiniBar current={d.currentPct} target={d.targetPct} color={bktColor} />
          ...
          <View style={ac.allMSRow}>
            {MILESTONES.map((m) => {
              ...
              const totalTarget = (
                (m.targets.bkt1 * (item.bktDetails.bkt1?.total || 0)) +
                (m.targets.bkt2 * (item.bktDetails.bkt2?.total || 0)) +
                (m.targets.bkt3 * (item.bktDetails.bkt3?.total || 0))
              ) / 100;
              ...
              return (
                <View
                  ...
                  style={[
                    ...
                    isPast     && { opacity: 0.4 },
                    achieved   && { backgroundColor: Colors.success + "15", borderColor: Colors.success },
                  ]}
                >
                  <Text style={[ac.msChipDay, isCurrent && { color: Colors.accent }, achieved && { color: Colors.success }]}>
                    D{m.day}
                  </Text>
                  <Text style={[ac.msChipLabel, isCurrent && { color: Colors.accent }, achieved && { color: Colors.success }]}>
                    {m.label}
                  </Text>
                </View>
              );
...
export default function AdminDRRScreen() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn:  () => api.admin.getBktPerfSummary(),
    ...
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      <View style={[s.topBar, { paddingTop: Platform.OS === "web" ? 67 : 12 }]}>
        ...
        <View style={s.teamStatsRow}>
          <View style={s.teamStat}>
            <Text style={s.teamStatLabel}>COLLECTED</Text>
            <Text style={[s.teamStatValue, { color: Colors.success }]}>{fmt(teamPaid)}</Text>
          </View>
          <View style={s.teamStat}>
            <Text style={s.teamStatLabel}>TOTAL POS</Text>
            <Text style={s.teamStatValue}>{fmt(teamPos)}</Text>
          </View>
          <View style={s.teamStat}>
            <Text style={s.teamStatLabel}>ON TARGET</Text>
            <Text style={[s.teamStatValue, { color: Colors.success }]}>{onTarget}/{agents.length}</Text>
          </View>
        </View>
          ...
          {MILESTONES.map((m) => {
            ...
            return (
              <View
                ...
                style={[
                  s.msRef,
                  isCurrent && { backgroundColor: Colors.accent + "20", borderColor: Colors.accent + "60" },
                  isPast     && { opacity: 0.4 },
                ]}
              >
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={s.count}>{agents.length} agent{agents.length !== 1 ? "s" : ""}</Text>
          }
          renderItem={({ item }) => <AgentDRRCard item={item} today={today} />}
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="bar-chart-outline" size={48} color={Colors.textMuted} />
              <Text style={s.emptyText}>No DRR data available yet</Text>
            </View>
          }
        />
      )}

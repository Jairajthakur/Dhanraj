...
async function apiRequest(method: string, route: string, data?: any) {
  ...
  const res = await fetch(url, { method, headers, credentials: "include", body: data ? JSON.stringify(data) : undefined });
...
function createFormData(file: any, extraData?: Record<string, any>) {
  ...
  form.append("file", { uri: file.uri, name: file.name || "file.xlsx", type: file.mimeType || file.type || "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" } as any);
...
async function uploadScreenshot(route: string, file: any, extraFields?: Record<string, any>) {
  ...
  const token = await tokenStore.get();
  const form = new FormData();
  form.append("screenshot", { uri: file.uri, name: file.name || "screenshot.jpg", type: file.mimeType || file.type || "image/jpeg" } as any);
  if (extraFields) Object.keys(extraFields).forEach((key) => form.append(key, String(extraFields[key])));
  const r = await fetch(url, { method: "POST", credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: form });
  const json = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(json.message || `HTTP ${r.status}`);
  return json;
}
...
export const api = {
  ...
  getBktCases: (category?: string) => apiRequest("GET", `/api/bkt-cases${category ? `?category=${category}` : ""}`),
  getStats: () => apiRequest("GET", "/api/stats"),
  updateFeedback: (id: number, data: any) => apiRequest("PUT", `/api/cases/${id}/feedback`, data),
  updateBktFeedback: (id: number, data: any) => apiRequest("PUT", `/api/bkt-cases/${id}/feedback`, data),
  getTodayPtp: () => apiRequest("GET", "/api/today-ptp"),
  addExtraNumber: (id: number, number: string, table: string) => apiRequest("POST", `/api/cases/${id}/extra-numbers`, { number, table }),
  removeExtraNumber: (id: number, number: string, table: string) => apiRequest("DELETE", `/api/cases/${id}/extra-numbers`, { number, table }),

  // ── Field visits ──────────────────────────────────────────────────────────
  getFieldVisits: (caseId: number) => apiRequest("GET", `/api/cases/${caseId}/visits`),
  recordFieldVisit: async (
    caseId: number,
    data: { lat: number; lng: number; accuracy?: number; case_type?: string; photo?: { uri: string; name?: string; mimeType?: string } | null; visit_outcome?: string; visit_remarks?: string; }
  ) => {
    if (data.photo) {
      const baseUrl = getApiUrl();
      ...
      if (Platform.OS === "web") {
        ...
        form.append("photo", { uri: data.photo.uri, name: data.photo.name || "visit.jpg", type: data.photo.mimeType || "image/jpeg" } as any);
      }
      const r = await fetch(url, { method: "POST", credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: form });
      const json = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(json.message || `HTTP ${r.status}`);
    ...
    return apiRequest("POST", `/api/cases/${caseId}/visit`, { lat: data.lat, lng: data.lng, accuracy: data.accuracy, case_type: data.case_type, visit_outcome: data.visit_outcome, visit_remarks: data.visit_remarks });
  },
  ...
  uploadDepositScreenshot: (depositId: number, file: any) => uploadScreenshot(`/api/required-deposits/${depositId}/screenshot`, file),

  // ── FOS Depositions ───────────────────────────────────────────────────────
  getFosDepositions: () => apiRequest("GET", "/api/fos-depositions"),
  fosDepositPayCash: (id: number, cashAmount: number) => apiRequest("POST", `/api/fos-depositions/${id}/pay-cash`, { cashAmount }),
  fosDepositPayOnline: async (id: number, file: any) => uploadScreenshot(`/api/fos-depositions/${id}/pay-online`, file),
  fosDepositPayBoth: async (id: number, cashAmount: number, onlineAmount: number, file?: any) => {
    ...
    if (file) form.append("screenshot", { uri: file.uri, name: file.name || "screenshot.jpg", type: file.mimeType || file.type || "image/jpeg" } as any);
    const r = await fetch(url, { method: "PUT", credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: form });
    const json = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(json.message || `HTTP ${r.status}`);
    return json;
  },
  ...
  requestReceipt: (caseId: number, data: { loan_no?: string; customer_name?: string; table_type?: string; notes?: string; emi_amount?: number; cbc?: number; lpp?: number; }) =>
  ...
  makeCall: (data: { customerPhone: string; agentName: string; caseId: string | number; loanNo: string; }) =>
  ...
  admin: {
    ...
    getStats:  (company?: string) => {
      ...
      return apiRequest("GET", `/api/admin/stats${qs}`);
    },
    getAgentStats: (agentId: number) => apiRequest("GET", `/api/admin/agent/${agentId}/stats`),
    ...
    getCases: (params?: { company?: string }) => {
      const qs = params?.company ? `?company=${encodeURIComponent(params.company)}` : "";
      return apiRequest("GET", `/api/admin/cases${qs}`);
    },
    getCasesByAgent: (agentId: number) => apiRequest("GET", `/api/admin/cases/agent/${agentId}`),
    createCase: (data: any) => apiRequest("POST", "/api/admin/cases", data),
    updateCaseStatus: (id: number, data: { status: string; rollback_yn?: boolean | null; table: "loan" | "bkt"; }) =>
      apiRequest("PUT", `/api/admin/cases/${id}/status`, data),
    reassignCase: (caseId: number, data: { to_agent_id: number; case_type?: string; reason?: string; }) =>
      apiRequest("PATCH", `/api/admin/cases/${caseId}/reassign`, data),
    getReassignLog: (caseId: number) => apiRequest("GET", `/api/admin/cases/${caseId}/reassign-log`),
    ...
    createRequiredDeposit: (data: { agentId: number; amount: number; description?: string; dueDate?: string; }) =>
    ...
    markCashCollected: (id: number) => apiRequest("PUT", `/api/admin/required-deposits/${id}/cash-collected`),
    verifyDeposit: (id: number)      => apiRequest("PUT", `/api/admin/required-deposits/${id}/verify`),

    // ── FOS Depositions ──────────────────────────────────────────────────────
    getFosDepositions: () => apiRequest("GET", "/api/admin/fos-depositions"),
    getFosDepositionsByAgent: (agentId: number) => apiRequest("GET", `/api/admin/fos-depositions/${agentId}`),
    createFosDeposition: (data: any) => apiRequest("POST", "/api/admin/fos-depositions", data),
    updateFosDepositionPayment: (id: number, data: any) => apiRequest("PUT", `/api/admin/fos-depositions/${id}/payment`, data),
    deleteFosDeposition: (id: number) => apiRequest("DELETE", `/api/admin/fos-depositions/${id}`),

    // ── Field Visits ─────────────────────────────────────────────────────────
    getAdminFieldVisits: (params?: { agent_id?: number; date?: string; company?: string }) => {
      const qs = new URLSearchParams();
    ...
    // ── BKT perf ─────────────────────────────────────────────────────────────
    getBktPerfSummary: (company?: string) => {
      const qs = company ? `?company=${encodeURIComponent(company)}` : "";
      return apiRequest("GET", `/api/admin/bkt-perf-summary${qs}`);
    },
    ...
    exportPtp: async () => {
      const baseUrl = getApiUrl();
      const token = await tokenStore.get();
      const r = await fetch(buildUrl("/api/admin/ptp-export", baseUrl), { credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
    ...
    exportFeedback: async () => {
      const baseUrl = getApiUrl();
      const token = await tokenStore.get();
      const r = await fetch(buildUrl("/api/admin/feedback-export", baseUrl), { credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.blob();
    },
    exportFosDepositions: async () => {
      const baseUrl = getApiUrl();
      const token = await tokenStore.get();
      const r = await fetch(buildUrl("/api/admin/fos-depositions-export", baseUrl), { credentials: "include", headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.blob();
    },

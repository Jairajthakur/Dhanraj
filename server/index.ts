...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/cases/:id/feedback", requireAuth, async (req, res) => {
    try {
      ...
      const ynVal = toBool(rollback_yn);
  ...
  app.get("/api/admin/feedback-export", requireAdmin, async (req, res) => {
    try {
      const result = await storage.query(`
        SELECT fa.name AS fos_name, lc.loan_no, lc.app_id, lc.customer_name,
               lc.bkt::text AS bkt, lc.pro,
               lc.customer_available, lc.vehicle_available, lc.third_party,
               lc.feedback_code, lc.latest_feedback AS details_feedback,
               lc.ptp_date::text AS ptp_date, lc.projection,
               lc.non_starter, lc.kyc_purchase, lc.workable,
               lc.feedback_date, lc.status
        FROM loan_cases lc
        LEFT JOIN fos_agents fa ON lc.agent_id = fa.id
        WHERE lc.feedback_date IS NOT NULL
        UNION ALL
        SELECT fa.name AS fos_name, bc.loan_no, bc.app_id, bc.customer_name,
               bc.case_category AS bkt, bc.pro,
               bc.customer_available, bc.vehicle_available, bc.third_party,
               bc.feedback_code, bc.latest_feedback AS details_feedback,
               bc.ptp_date::text AS ptp_date, bc.projection,
               bc.non_starter, bc.kyc_purchase, bc.workable,
               bc.feedback_date, bc.status
        FROM bkt_cases bc
        LEFT JOIN fos_agents fa ON bc.agent_id = fa.id
        WHERE bc.feedback_date IS NOT NULL
      ...
      const rows = result.rows.map((r: any) => ({
        "Allu Date":        r.feedback_date ? new Date(r.feedback_date).toLocaleDateString("en-IN") : "",
        "Loan No":          r.loan_no || "",
        "App ID":           r.app_id || "",
        ...
        "Pro":              r.pro || "",
        "FOS Name":         r.fos_name || "",
        "Customer Avail":   yn(r.customer_available),
        "Vehicle Avail":    yn(r.vehicle_available),
        "Third Party":      yn(r.third_party),
        "FEEDBACK":         r.feedback_code || "",
        "Details FEEDBACK": r.details_feedback || "",
        "PTP DATE":         r.ptp_date || "",
        "Projection":       r.projection || "",
        "NON_START":        yn(r.non_starter),
        "KYC PURCH":        yn(r.kyc_purchase),
      ...
      const exportRows = rows.length ? rows : [{ "Allu Date": "No feedback found" }];
      ws.columns = Object.keys(exportRows[0]).map(key => ({ header: key, key, width: 18 }));
